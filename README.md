...
5、不需内存：segmentedBitonicSort不调用任何函数（包括C/C++标准库函数）， 
不使用全局变量，所有局部变量都是int、float或指针类 型，C++程序不使用new 
关键字。 

6、绝对鲁棒：在输入数据中包含NaN时（例如sqrt(-1.f)），保证除NaN以外 的数 
据正确排序，NaN的个数保持不变。 
...
### 双调排序
见[博文](http://blog.csdn.net/sunmenggmail/article/details/42869235)
...
3
请输入需要排序的浮点数序列：（输入8个数)
1.4 0.3 1.1 0.9 0.3 1.3 NaN 1.6
请输入序列的段编号：(输入8个数)
0 0 0 1 1 2 2 2
请输入序列的段开始值：(输入4个数，注意最后一个数应该等于序列个数)
0 3 5 8
输入正确，排序结果是：
0.3 1.1 1.4 0.3 0.9 1.3 1.6 nan
```
...
3
请输入需要排序的浮点数序列：（输入8个数)
1.4 0.3 1.1 0.9 0.3 1.3 0.8 1.6
请输入序列的段编号：(输入8个数)
0 0 0 1 1 2 2 2
请输入序列的段开始值：(输入4个数，注意最后一个数应该等于序列个数)
0 3 5 9
输入错误，序列开始码最后一位值与序列个数不一致。
```
